...
from gptbot import add_message_to_the_conversation, main, setup_openAI_client, get_user_input, print_ai_output, validate_environment_variables, validate_system_arguments, setup_and_validate_environment
...
def test_main_api_connection_error():
    with patch("sys.argv", ["gptbot.py", "Tom"]), patch(
        ...
        with pytest.raises(SystemExit) as e:
            main()
        assert str(e.value) == f"Network error occurred: {openai.APIConnectionError(request='request')}"
...
def test_validate_environment_variables():
    ...
    assert str(e.value) == "API key or organization not provided in environment variables."
